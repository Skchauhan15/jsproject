  ...
  var productload = () => {
    $.get(
      ...
      function (productList, status) {
        ...
        for (var z = 0; z < cardDiv.length; z++) {
          cardDiv[z].addEventListener("click", function () {
            ...
            for (var k = 0; k < big.length; k++) {
              if (k == 0) {
                big[k].classList.add("actives");
              }
              big[k].addEventListener("click", function (e) {
                big_img.src = e.target.src;
                var x = document.getElementsByClassName("actives");
                if (x.length > 0) {
                  x[0].classList.remove("actives");
                }
            ...
            addcart_button.addEventListener("click", function () {
              ...
              var recieved_cardItems = JSON.parse(
        ...
        //checkoutpage  start here  /////////////////////////////////
        var clickcount = 0;
        var cartIcon = document.getElementById("cartIcon");
        cartIcon.addEventListener("click", function () {
          clickcount++;
          if (clickcount == 1) {
            $("#checkout_page").prepend(
              `<h1 id="checkout_heading">Checkout</h1><div id="total_items_text">Total Items:</div>`
            );
            $("#checkout_page").append(
              `<div id="checkout_innerdiv"><div id="final_C1"></div><div id="final_C2"></div></div>`
            );
            ...
            $("#final_C2").append(
              '<div id="tm">Total Amount</div><spam id="Am">Amount: RS </span><span id="trs">0</span><button id="placeOrder">Place Order</button>'
            );
            var Tamount = 0;
            var quantity = 1;
            var products = [];
                ...
                for (var j in productList[recieved_cardItems[i]]) {
                  ...
                  if (j == "name") {
                    cartItemName.innerHTML +=
                      productList[recieved_cardItems[i]][j];
                  }
                  if (j == "price") {
                    cartItemRs.innerHTML +=
                      " Amount:Rs " + productList[recieved_cardItems[i]][j];
            ...
            // placeorder page start here//////////////////////////////////
            var placeorder = document.getElementById("placeOrder");
            placeorder.addEventListener("click", function () {
              document.getElementById("checkout_page").className = "hide";
              $("#orderplaced_page").append(
                '<div id=circle><i id="check" class="fa-solid fa-check fa-6x"></i></div><h1>Order Placed Successfully!!</h1>'
              );
              $("#orderplaced_page").append(
